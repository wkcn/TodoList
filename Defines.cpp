...
QDir directoryOf(const QString &subdir) {
  ...
  if (dir.dirName().toLower() == "debug" ||
      dir.dirName().toLower() == "release" || dir.dirName().toLower() == "bin")
    dir.cdUp();
...
bool Date::operator<(const Date &b) const { return intTime < b.intTime; }
...
Date GetDate(int offsetHour) {
  ...
  d.day = 1;
  int temp;
  while (t >= (temp = YearHaveDay(d.year))) {
    t -= temp;
    d.year++;
  }
  while (t >= (temp = MonthHaveDay(d.month, IsLeapYear(d.year)))) {
    t -= temp;
    d.month++;
  }
  d.day += t;
...
int Date::operator()() { return GetTime(); }
